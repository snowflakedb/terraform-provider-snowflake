...
pre-push: generate-all-config-model-builders generate-sdk generate-sdk-examples mod fmt generate-docs-additional-files generate-issue-labels docs lint-fix test-architecture ## Run a few checks and generators. It should be used only locally because it modifies or fixes the code.

pre-push-check: generate-all-config-model-builders-check generate-sdk-check generate-sdk-examples-check mod-check fmt-check generate-docs-additional-files-check generate-issue-labels-check docs-check lint test-architecture ## Run checks before pushing a change (docs, fmt, mod, etc.)
...
generate-sdk-check: ## Check that all generated SDK files (except tests) are up-to-date
...
.PHONY: build-local dev-setup dev-cleanup docs docs-check fmt fmt-check fumpt help install lint lint-fix mod mod-check pre-push pre-push-check sweep terraform-fmt terraform-fmt-check test test-acceptance uninstall-tf generate-sdk-check generate-sdk-examples-check
