---
page_title: "snowflake_iceberg_table_from_files Resource - terraform-provider-snowflake"
subcategory: "Preview"
description: |-
...
# snowflake_iceberg_table_from_files (Resource)
...
~> **Note** External changes to `metadata_file_path` are not detected. This value is not read back from Snowflake during refresh, so updates to the underlying Iceberg metadata file (for example, refreshing the table to a new metadata file outside of Terraform) will not produce a drift in the Terraform plan. After import, the first `terraform plan` may show a resource recreation for write-only non-alterable fields that are not reconciled with Snowflake. Run `terraform apply` once to sync these values into state.
...
```shell
terraform import snowflake_iceberg_table_from_files.example '"<database_name>"."<schema_name>"."<table_name>"'
```
