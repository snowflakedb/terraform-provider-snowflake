...
- `column` (Block List, Min: 1) Definitions of columns to create in the hybrid table. Minimum one required. Column structure changes (add/remove/rename columns) require table recreation, but comment changes can be updated in-place. (see [below for nested schema](#nestedblock--column))
