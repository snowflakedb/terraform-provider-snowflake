...
func CreateGrantDatabaseRole(ctx context.Context, d *schema.ResourceData, meta any) diag.Diagnostics {
	...
	if parentRoleName, ok := d.GetOk("parent_role_name"); ok && parentRoleName.(string) != "" {
		...
		req := sdk.NewGrantToShareDatabaseRoleRequest(databaseRoleIdentifier, shareIdentifier)
...
func DeleteGrantDatabaseRole(ctx context.Context, d *schema.ResourceData, meta any) diag.Diagnostics {
	...
	case "SHARE":
		...
		if err = revokeFromShareFunc(ctx, sdk.NewRevokeFromShareDatabaseRoleRequest(id, sharedId)); err != nil {
			return diag.FromErr(err)
		}
	}
	d.SetId("")
