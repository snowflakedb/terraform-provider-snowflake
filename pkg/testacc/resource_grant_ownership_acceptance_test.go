...
func TestAcc_GrantOwnership_OnAgent_ToAccountRole(t *testing.T) {
	cortexAgentId := testClient().Ids.RandomSchemaObjectIdentifier()
	cortexAgentFullyQualifiedName := cortexAgentId.FullyQualifiedName()

	cortexAgentCleanup := testClient().CortexAgent.CreateWithId(t, cortexAgentId)
	t.Cleanup(cortexAgentCleanup)
	...
	configVariables := config.Variables{
		"account_role_name":                 config.StringVariable(accountRoleName),
		"cortex_agent_fully_qualified_name": config.StringVariable(cortexAgentFullyQualifiedName),
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: ConfigurationDirectory("TestAcc_GrantOwnership/OnAgent_ToAccountRole"),
				ConfigVariables: configVariables,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "account_role_name", accountRoleName),
					resource.TestCheckResourceAttr(resourceName, "on.0.object_type", string(sdk.ObjectTypeAgent)),
					resource.TestCheckResourceAttr(resourceName, "on.0.object_name", cortexAgentFullyQualifiedName),
					resource.TestCheckResourceAttr(resourceName, "id", fmt.Sprintf("ToAccountRole|%s||OnObject|AGENT|%s", accountRoleFullyQualifiedName, cortexAgentFullyQualifiedName)),
					checkResourceOwnershipIsGranted(&sdk.ShowGrantOptions{
						To: &sdk.ShowGrantsTo{
							Role: accountRoleId,
						},
					}, sdk.ObjectTypeAgent, accountRoleName, cortexAgentFullyQualifiedName),
				),
			},
			{
				ConfigDirectory:   ConfigurationDirectory("TestAcc_GrantOwnership/OnAgent_ToAccountRole"),
...
func TestAcc_GrantOwnership_OnAllAgents_ToAccountRole(t *testing.T) {
	cortexAgentId := testClient().Ids.RandomSchemaObjectIdentifier()
	anotherCortexAgentId := testClient().Ids.RandomSchemaObjectIdentifierInSchema(cortexAgentId.SchemaId())
	schemaFullyQualifiedName := cortexAgentId.SchemaId().FullyQualifiedName()

	cortexAgentCleanup := testClient().CortexAgent.CreateWithId(t, cortexAgentId)
	t.Cleanup(cortexAgentCleanup)

	anotherCortexAgentCleanup := testClient().CortexAgent.CreateWithId(t, anotherCortexAgentId)
	t.Cleanup(anotherCortexAgentCleanup)
	...
	configVariables := config.Variables{
		"account_role_name":           config.StringVariable(accountRoleName),
		"schema_fully_qualified_name": config.StringVariable(schemaFullyQualifiedName),
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: ConfigurationDirectory("TestAcc_GrantOwnership/OnAllAgents_ToAccountRole"),
				ConfigVariables: configVariables,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "account_role_name", accountRoleName),
					resource.TestCheckResourceAttr(resourceName, "on.0.all.0.object_type_plural", string(sdk.PluralObjectTypeAgents)),
					resource.TestCheckResourceAttr(resourceName, "on.0.all.0.in_schema", schemaFullyQualifiedName),
					resource.TestCheckResourceAttr(resourceName, "id", fmt.Sprintf("ToAccountRole|%s||OnAll|AGENTS|InSchema|%s", accountRoleFullyQualifiedName, schemaFullyQualifiedName)),
					checkResourceOwnershipIsGranted(&sdk.ShowGrantOptions{
						To: &sdk.ShowGrantsTo{
							Role: accountRoleId,
						},
					}, sdk.ObjectTypeAgent, accountRoleName, cortexAgentId.FullyQualifiedName(), anotherCortexAgentId.FullyQualifiedName()),
				),
			},
			{
				ConfigDirectory:   ConfigurationDirectory("TestAcc_GrantOwnership/OnAllAgents_ToAccountRole"),
