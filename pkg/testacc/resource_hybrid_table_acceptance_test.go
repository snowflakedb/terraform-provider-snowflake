...
// Reordering unique_constraint blocks textually must be a no-op now that
// unique_constraint is a TypeSet. Two semantically-equivalent configs that
// differ only in block order should produce an empty plan.
func TestAcc_HybridTable_UniqueConstraint_ReorderIsNoOp(t *testing.T) {
	id := testClient().Ids.RandomSchemaObjectIdentifier()
	cols := []sdk.TableColumnSignature{
		{Name: "ID", Type: testdatatypes.DataTypeInteger},
		{Name: "NAME", Type: testdatatypes.DataTypeVarchar},
	...
	uniqueOnName := tfconfig.MapVariable(map[string]tfconfig.Variable{
		"columns": tfconfig.ListVariable(tfconfig.StringVariable("NAME")),
	})
	uniqueOnEmail := tfconfig.MapVariable(map[string]tfconfig.Variable{
		"columns": tfconfig.ListVariable(tfconfig.StringVariable("EMAIL")),
	})

	model1 := model.HybridTableFromId("test", id, cols, pk).
		WithUniqueConstraintValue(tfconfig.SetVariable(uniqueOnName, uniqueOnEmail))
	model2 := model.HybridTableFromId("test", id, cols, pk).
		WithUniqueConstraintValue(tfconfig.SetVariable(uniqueOnEmail, uniqueOnName))
...
// Reordering foreign_key blocks textually must be a no-op now that
// foreign_key is a TypeSet. The child has two FKs to the same parent column
// from two different local columns; swapping the textual order must not
// produce a plan.
func TestAcc_HybridTable_ForeignKey_ReorderIsNoOp(t *testing.T) {
	parentId := testClient().Ids.RandomSchemaObjectIdentifier()
	testClient().HybridTable.CreateWithRequest(t, parentId, sdk.HybridTableColumnsConstraintsAndIndexesRequest{
		Columns: []sdk.HybridTableColumnRequest{
			*sdk.NewHybridTableColumnRequest("ID", sdk.DataType("INTEGER")).
	...
	cols := []sdk.TableColumnSignature{
		{Name: "ID", Type: testdatatypes.DataTypeInteger},
		{Name: "PARENT_REF_A", Type: testdatatypes.DataTypeInteger},
		{Name: "PARENT_REF_B", Type: testdatatypes.DataTypeInteger},
	}
	pk := []sdk.TableColumnSignature{{Name: "ID"}}

	fkBlock := func(localCol string) tfconfig.Variable {
		return tfconfig.ObjectVariable(map[string]tfconfig.Variable{
			"columns": tfconfig.ListVariable(tfconfig.StringVariable(localCol)),
	...
	fkA := fkBlock("PARENT_REF_A")
	fkB := fkBlock("PARENT_REF_B")

	model1 := model.HybridTableFromId("test", id, cols, pk).
		WithForeignKeyValue(tfconfig.SetVariable(fkA, fkB))
	model2 := model.HybridTableFromId("test", id, cols, pk).
		WithForeignKeyValue(tfconfig.SetVariable(fkB, fkA))
