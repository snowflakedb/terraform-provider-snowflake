...
func TestAcc_GrantAccountRole_IdentifierQuotingDiffSuppression(t *testing.T) {
	...
	roleModel := model.AccountRole("role", quotedRoleId)
	parentRoleModel := model.AccountRole("parent_role", quotedParentRoleId)
	grantModel := model.GrantAccountRole("test", roleId.Name()).
		WithRoleNameValue(accconfig.UnquotedWrapperVariable(fmt.Sprintf("%s.name", roleModel.ResourceReference()))).
		WithParentRoleNameValue(accconfig.UnquotedWrapperVariable(fmt.Sprintf("%s.name", parentRoleModel.ResourceReference())))
		...
		Steps: []resource.TestStep{
			{
				...
				Config:            providerConfig + accconfig.FromModels(t, roleModel, parentRoleModel, grantModel),
			},
			{
				PreConfig:                func() { UnsetConfigPathEnv(t) },
				ProtoV6ProviderFactories: TestAccProtoV6ProviderFactories,
