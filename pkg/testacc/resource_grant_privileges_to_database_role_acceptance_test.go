		...
		Steps: []resource.TestStep{
			{
				...
				Config:            accconfig.FromModels(t, providerModel, privateKeyVar, passphraseVar) + grantPrivilegesToDatabaseRoleBasicConfig(quotedDatabaseRoleId, quotedSchemaId),
				Check: resource.ComposeAggregateTestCheckFunc(
		...
		Steps: []resource.TestStep{
			{
				PreConfig:         func() { SetV097CompatibleConfigWithServiceUserPathEnv(t) },
				ExternalProviders: ExternalProviderWithExactVersion("0.94.1"),
				Config:            accconfig.FromModels(t, providerModel, privateKeyVar, passphraseVar) + grantPrivilegesToDatabaseRoleBasicConfig(unquotedDatabaseRoleId, unquotedSchemaId),
				Check: resource.ComposeAggregateTestCheckFunc(
		...
		Steps: []resource.TestStep{
			{
				...
				Config:            accconfig.FromModels(t, providerModel, privateKeyVar, passphraseVar) + grantPrivilegesToDatabaseRoleOnFutureInDatabaseConfig(databaseRoleId, []string{"USAGE"}, sdk.PluralObjectTypeModels, databaseRoleId.DatabaseName()),
