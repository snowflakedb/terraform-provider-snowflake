...
import (
	...
	_ "github.com/hashicorp/terraform-plugin-testing/plancheck"
	"github.com/hashicorp/terraform-plugin-testing/tfversion"
)

func TestAcc_SemanticView_basic(t *testing.T) {
	basicID, completeID := testClient().Ids.RandomSchemaObjectIdentifier(), testClient().Ids.RandomSchemaObjectIdentifier()
		...
		Steps: []resource.TestStep{
			// create with only required attributes
			{
				...
				Check: assertThat(t,
					resourceassert.SemanticViewResource(t, modelBasic.ResourceReference()).
						HasNameString(basicID.Name()).
						HasDatabaseString(basicID.DatabaseName()).
						HasSchemaString(basicID.SchemaName()).
						HasCommentString("").
						HasFullyQualifiedNameString(basicID.FullyQualifiedName()),
			...
			// import minimal state
			{
				...
				ImportStateCheck: assertThatImport(t,
					resourceassert.ImportedSemanticViewResource(t, helpers.EncodeResourceIdentifier(basicID)).
						HasNameString(basicID.Name()).
						HasDatabaseString(basicID.DatabaseName()).
						HasSchemaString(basicID.SchemaName()).
						HasCommentString("").
						HasFullyQualifiedNameString(basicID.FullyQualifiedName()),
					resourceshowoutputassert.ImportedSemanticViewShowOutput(t, helpers.EncodeResourceIdentifier(basicID)).
						HasCreatedOnNotEmpty().
						HasName(basicID.Name()).
						HasDatabaseName(basicID.DatabaseName()).
						HasSchemaName(basicID.SchemaName()).
			...
			// add optional attributes
			{
				...
				Check: assertThat(t,
					resourceassert.SemanticViewResource(t, modelComplete.ResourceReference()).
						HasNameString(completeID.Name()).
						HasDatabaseString(completeID.DatabaseName()).
						HasSchemaString(completeID.SchemaName()).
						HasCommentString(comment).
						HasFullyQualifiedNameString(completeID.FullyQualifiedName()),
					resourceshowoutputassert.SemanticViewShowOutput(t, modelComplete.ResourceReference()).
						HasCreatedOnNotEmpty().
						HasName(completeID.Name()).
						HasDatabaseName(completeID.DatabaseName()).
						HasSchemaName(completeID.SchemaName()).
						HasOwner(snowflakeroles.Accountadmin.Name()).
						HasOwnerRoleType("ROLE").
						HasComment(comment).
						HasExtension(""),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.#", "36")),
					...
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.28.object_name", "WF1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.28.parent_entity", "LT1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.28.property", "TABLE")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.28.property_value", "LT1")),

					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.29.object_kind", "METRIC")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.29.object_name", "WF1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.29.parent_entity", "LT1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.29.property", "EXPRESSION")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.29.property_value", "sum(lt1.se1) OVER (PARTITION BY lt1.d1)")),

					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.30.object_kind", "METRIC")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.30.object_name", "WF1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.30.parent_entity", "LT1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.30.property", "DATA_TYPE")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.30.property_value", "NUMBER(38,0)")),

					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.31.object_kind", "METRIC")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.31.object_name", "WF1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.31.parent_entity", "LT1")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.31.property", "ACCESS_MODIFIER")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.31.property_value", "PUBLIC")),
					...
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.32.property", "BASE_TABLE_DATABASE_NAME")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.32.property_value", table2.DatabaseName)),
					...
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.33.property", "BASE_TABLE_SCHEMA_NAME")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.33.property_value", table2.SchemaName)),

					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.34.object_kind", "TABLE")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.34.object_name", "LT2")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.34.parent_entity", "")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.34.property", "BASE_TABLE_NAME")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.34.property_value", table2.Name)),

					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.35.object_kind", "TABLE")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.35.object_name", "LT2")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.35.parent_entity", "")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.35.property", "PRIMARY_KEY")),
					assert.Check(resource.TestCheckResourceAttr(modelComplete.ResourceReference(), "describe_output.35.property_value", "[\"A1\"]")),
				),
			},
			// alter
			{
			...
			{
				PreConfig: func() {
					testClient().SemanticView.Alter(t, sdk.NewAlterSemanticViewRequest(completeID).WithSetComment(comment))
				},
				Config: accconfig.FromModels(t, modelCompleteWithDifferentValues),
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PreApply: []plancheck.PlanCheck{
			...
			{
				PreConfig: func() {
					_, semanticViewCleanup := testClient().SemanticView.CreateWithRequest(t, sdk.NewCreateSemanticViewRequest(completeID, []sdk.LogicalTableRequest{*lt1Request, *lt2Request}).
