...
If there's a need to support more object types, please open an issue or a PR. TODO: Contributing guide.

## Usage

Example usage will be based on the grant resources, to get find more information about other supported object types,
...
- Snowflake account with an ability to create roles and grant privileges (for simplicity, an account with ACCOUNTADMIN role would be the best choice).
...
### Query Snowflake and save the output

Let's say we will only focus on the privileges granted to the account role on the current account.
To set up the necessary objects, run the following commands in Snowflake:
```sql
...
### Generate resources and import statements based on the Snowflake output

To use the script with the saved output, run the following command in the terminal:

```shell
go run github.com/Snowflake-Labs/terraform-provider-snowflake/pkg/scripts/migration_script@add-import-generating-part-1 -import=block grants < example.csv
```

> For more details on the command-line options, run:
> ```shell
> go run github.com/Snowflake-Labs/terraform-provider-snowflake/pkg/scripts/migration_script@add-import-generating-part-1 -h
> ```
...
```shell
go run github.com/Snowflake-Labs/terraform-provider-snowflake/pkg/scripts/migration_script@add-import-generating-part-1 -import=block grants < example.csv > output.tf
```

### Get the generated resources and import them to the state
...
Then run `terraform plan`, you should the following output similar to this:
```
  # snowflake_grant_privileges_to_account_role.test_resource_name_on_account will be imported
  ...
  # snowflake_grant_privileges_to_account_role.test_resource_name_on_account_2 will be imported
    resource "snowflake_grant_privileges_to_account_role" "test_resource_name_on_account_2" {
...
```
which means the resources have been successfully imported into the state, and you can proceed to import other resources.
