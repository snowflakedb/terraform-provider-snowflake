# =============================================================================
# Account Roles Data Source - Fetch roles from Snowflake and generate CSV
# =============================================================================
# This file fetches the test account roles created by objects_def.tf and
...
#   3. Test migration: go run .. -import=block account_roles < objects.csv
# =============================================================================

# Fetch all test account roles (filter by MIGRATION_TEST_ROLE_ prefix)
data "snowflake_account_roles" "test_roles" {
  like = "MIGRATION_TEST_ROLE_%"
}

locals {
  # Transform each role to use show_output fields directly
  roles_flattened = [
    for role in data.snowflake_account_roles.test_roles.account_roles :
    role.show_output[0]
  ]
  ...
  roles_csv_content = length(local.roles_flattened) > 0 ? join("\n", concat([local.roles_csv_header], local.roles_csv_rows)) : "# No roles found matching MIGRATION_TEST_ROLE_%"
}
...
output "roles_found" {
  description = "Number of test roles found"
  value       = length(local.roles_flattened)
}

output "role_names" {
  description = "Names of test roles found"
