...
// returns []NetworkPolicyProperty. Fields are the property .Value strings so
// today’s TypeString describe_output keys generate natively.
var networkPolicyDetailsDef = g.PlainStruct("NetworkPolicyDetails").
	Text("AllowedIpList").
	Text("BlockedIpList").
	Text("AllowedNetworkRuleList").
	Text("BlockedNetworkRuleList")
