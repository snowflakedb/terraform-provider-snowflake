...
func TestInt_SemanticView(t *testing.T) {
	...
	t.Run("create: with private facts and metrics", func(t *testing.T) {
		...
		privateMetric := sdk.NewMetricDefinitionRequest().WithSemanticExpression(*metricSemanticExpression).WithPrivateMetric()
		...
		fact := sdk.NewFactDefinitionRequest().WithSemanticExpression(*factSemanticExpression).WithPrivateFact()
