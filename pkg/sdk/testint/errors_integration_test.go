...
func TestInt_ShowSchemaObjectInNonExistingDatabase(t *testing.T) {
	testCases := []struct {
		...
		{ObjectType: sdk.ObjectTypeNotebook, ExpectedErr: sdk.ErrObjectNotFound, ShowFn: schemaObjectShowByIDWrapper(testClient(t).Notebooks.ShowByID)},
	}
...
func TestInt_DropSchemaObjectInNonExistingDatabase(t *testing.T) {
	...
	testCases := []struct {
		...
		{ObjectType: sdk.ObjectTypeNotebook, ExpectedErr: sdk.ErrObjectNotFound, DropFn: schemaObjectDropWrapper(testClient(t).Notebooks.Drop, sdk.NewDropNotebookRequest(id).WithIfExists(true))},
	}
