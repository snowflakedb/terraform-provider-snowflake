...
func TestInt_ApiIntegrations(t *testing.T) {
	...
	// logProperties logs all describe properties for discovery when keys are unknown.
	logProperties := func(t *testing.T, id sdk.AccountObjectIdentifier) {
		t.Helper()
		props, err := client.ApiIntegrations.Describe(context.Background(), id)
		if err != nil {
			t.Logf("[DISCOVERY] describe failed: %v", err)
			return
		}
		for _, p := range props {
			t.Logf("[DISCOVERY] property=%q type=%q value=%q default=%q", p.Name, p.Type, p.Value, p.Default)
		}
	}
	...
	t.Run("create: git https api with token auth (all secrets)", func(t *testing.T) {
		request := newGitTokenRequest(t)
		integration := createWithRequest(t, request)
		assertShowResult(t, integration, "")

		logProperties(t, integration.ID())
	...
	t.Run("create: git https api with GitHub App auth", func(t *testing.T) {
		request := newGitGithubAppRequest(t)
		integration := createWithRequest(t, request)
		assertShowResult(t, integration, "")
	...
	t.Run("create: git https api with private link", func(t *testing.T) {
		request := newGitPrivateLinkRequest(t)
		integration := createWithRequest(t, request)
		assertShowResult(t, integration, "")
	...
	t.Run("create: external mcp with dynamic client auth", func(t *testing.T) {
		request := newMcpDynamicClientRequest(t)
		integration := createWithRequest(t, request)
		assertShowResult(t, integration, "")
	...
	t.Run("alter set: git github app - re-authenticate", func(t *testing.T) {
		integration := createWithRequest(t, newGitGithubAppRequest(t))

		err := client.ApiIntegrations.Alter(ctx, sdk.NewAlterApiIntegrationRequest(integration.ID()).
			WithSet(*sdk.NewApiIntegrationSetRequest().
				WithGitHttpsApiGithubAppParams(*sdk.NewSetGitHttpsApiGithubAppParamsRequest()).
				WithEnabled(true).
				WithComment("updated")))
		require.NoError(t, err)
	...
	t.Run("alter set: git oauth2 - update credentials", func(t *testing.T) {
		integration := createWithRequest(t, newGitOAuth2Request(t))

		newAuth := sdk.NewOAuth2GitUserAuthenticationRequest(oauthAuthorizationEndpoint, oauthTokenEndpoint, "new-client-id", "new-secret").
			WithOauthAccessTokenValidity(7200)
		err := client.ApiIntegrations.Alter(ctx, sdk.NewAlterApiIntegrationRequest(integration.ID()).
			WithSet(*sdk.NewApiIntegrationSetRequest().
				WithGitHttpsApiOAuth2Params(*sdk.NewSetGitHttpsApiOAuth2ParamsRequest().WithApiUserAuthentication(*newAuth)).
				WithComment("updated")))
		require.NoError(t, err)
	...
	t.Run("alter set: external mcp dynamic client - update resource url", func(t *testing.T) {
		integration := createWithRequest(t, newMcpDynamicClientRequest(t))

		newAuth := sdk.NewDynamicClientMcpUserAuthenticationRequest("https://other-resource.example.com")
		err := client.ApiIntegrations.Alter(ctx, sdk.NewAlterApiIntegrationRequest(integration.ID()).
			WithSet(*sdk.NewApiIntegrationSetRequest().
				WithExternalMcpDynamicClientParams(*sdk.NewSetExternalMcpDynamicClientParamsRequest().WithApiUserAuthentication(*newAuth)).
				WithComment("updated")))
		require.NoError(t, err)
