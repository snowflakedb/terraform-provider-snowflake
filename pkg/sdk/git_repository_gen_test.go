...
func TestGitRepositories_Create(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		opts := defaultOpts()
		opts.OrReplace = Bool(true)
		assertOptsValidAndSQLEquals(t, opts, "CREATE OR REPLACE GIT REPOSITORY %s ORIGIN = '%s' API_INTEGRATION = %s", id.FullyQualifiedName(), origin, apiIntegrationId.Name())
	})
	...
	t.Run("all options", func(t *testing.T) {
		...
		assertOptsValidAndSQLEquals(t, opts, "CREATE GIT REPOSITORY IF NOT EXISTS %s ORIGIN = '%s' API_INTEGRATION = %s GIT_CREDENTIALS = %s COMMENT = '%s'", id.FullyQualifiedName(), origin, apiIntegrationId.FullyQualifiedName(), gitCredentialsId.FullyQualifiedName(), "comment")
	})
}
...
func TestGitRepositories_Alter(t *testing.T) {
	...
	t.Run("validation: exactly one field from [opts.Set opts.Unset] should be present", func(t *testing.T) {
		...
		assertOptsInvalidJoinedErrors(t, opts, errExactlyOneOf("AlterGitRepositoryOptions", "Set", "Unset"))
	})
